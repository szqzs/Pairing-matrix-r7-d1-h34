"""Semantic-key scaffold for c18 even rows against one-f H62 tests."""
...
def enumerate_c18_even_one_f_keys(
    *,
    ...
    """Enumerate unique f-only semantic keys for the one-f test matrix."""
    ...
    columns, test_column_indices = _select_one_f_columns(
        config,
    ...
    for row in rows:
        for column in columns:
            ...
            key = f_only_key_id(total_f_exp, total_a_exp)
    ...
    return {
        "kind": "c18_even_one_f_semantic_key_enumeration",
        "row_count": len(rows),
...
def evaluate_f_only_key(
    ...
    *,
    ...
    """Evaluate one f-only semantic key modulo a prime."""
    ...
    if normalized in {"batched", "semantic-batched"}:
        return f_only_pairing_total_batched_mod(config, total, prime=p)
    if normalized == "moment":
        return f_only_pairing_total_moment_mod(config, total, prime=p)
    raise ValueError("method must be moment, batched, or semantic-batched")


def _select_one_f_columns(
